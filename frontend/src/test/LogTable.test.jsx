...
describe('handleSelectAllClick', () => {
    ...
    // it('should select all rows when no rows are selected and checkbox is checked', () => {
    //     const selectMultipleRowsMock = jest.fn();
    //     const { rerender } = render(
    //         <RowsProvider value={{ rows: mockRows }}>
    //             <SelectedRowsProvider value={{ selectedRows: [], selectMultipleRows: selectMultipleRowsMock }}>
    //                 <LogTable />
    ...
    //     // Симулируем клик по чекбоксу, чтобы выбрать все строки
    //     fireEvent.click(selectAllCheckbox);

    //     // Проверяем, что selectMultipleRows была вызвана с правильными ID строк
    //     expect(selectMultipleRowsMock).toHaveBeenCalledWith([1, 2]);

    //     // Повторный рендеринг с новым состоянием
    //     rerender(
    //         <RowsProvider value={{ rows: mockRows }}>
    //             <SelectedRowsProvider value={{ selectedRows: [1, 2], selectMultipleRows: selectMultipleRowsMock }}>
    //                 <LogTable />
    ...
    // it('should deselect all rows when some rows are selected and checkbox is checked', () => {
    //     const selectMultipleRowsMock = jest.fn();
    //     const { rerender } = render(
    //         <RowsProvider value={{ rows: mockRows }}>
    //             <SelectedRowsProvider value={{ selectedRows: [1], selectMultipleRows: selectMultipleRowsMock }}>
    //                 <LogTable />
    //             </SelectedRowsProvider>
    //         </RowsProvider>
    //     );
    ...
    //     // Симулируем клик по чекбоксу, чтобы снять все выделения
    //     fireEvent.click(selectAllCheckbox);

    //     // Проверяем, что selectMultipleRows была вызвана с пустым массивом
    //     expect(selectMultipleRowsMock).toHaveBeenCalledWith([]);
    ...
    // it('should select all rows when some rows are selected and checkbox is unchecked', () => {
    //     const selectMultipleRowsMock = jest.fn();
    //     const { rerender } = render(
    //         <RowsProvider value={{ rows: mockRows }}>
    //             <SelectedRowsProvider value={{ selectedRows: [1], selectMultipleRows: selectMultipleRowsMock }}>
